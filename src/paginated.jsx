...
function Products() {
    ...
    const skip = parseInt(searchParams.get('skip') || 0); // Initial limit set to 4
    ...
    const { data: products } = useQuery({
        ...
        queryFn: async () => {
            const data = await fetch(`https://dummyjson.com/products/search?limit=${limit}&skip=${skip}&q=${q}`)
            .then((res) => res.json());
